<%@ Page Title="" Language="C#" EnableEventValidation="false" MasterPageFile="~/iljin.Master" AutoEventWireup="true" CodeBehind="Frmunitprice.aspx.cs" Inherits="iljin.Frmunitprice" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
        function sub_disp(code, popNo) {
            if (popNo == '0') {
                var url = "/base/popUp/popunitprice_update.aspx?code=" + code;
                var name = "_blank"
                var popupX = (window.screen.width / 2) - (450 / 2);
                var popupY = (window.screen.height / 2) - (300 / 2);
                window.open(url, name, 'status=no, width=450, height=300, left=' + popupX + ',top=' + popupY);

            }
            else {
                var url = "/base/popUp/popunitprice_updatelist.aspx?code=" + code;
                var name = "_blank"
                var popupX = (window.screen.width / 2) - (500 / 2);
                var popupY = (window.screen.height / 2) - (375 / 2);
                window.open(url, name, 'status=no, width=500, height=375, left=' + popupX + ',top=' + popupY);
            }
        }
        function refresh() {
...
<asp:Panel ID="defaultPanel1" runat="server" DefaultButton="btn_default">
    ...
    <article class="conts_inner">
        <h2 class="conts_tit">
            <asp:Label ID="m_title" runat="server" Text="기준정보관리 ::> 단가관리"></asp:Label>
            ...
            <table class="grtable_th">
                <thead>
                    <tr>
                        ...
                        <th class="mWt10p">제품코드</th>
                        <th class="mWt20p">품명</th>
                        <th class="mWt12p">제품구분</th>
                        <th class="mWt8p">두께</th>
                        <th class="mWt8p">폭</th>
                        <th class="mWt8p">길이</th>
                        <th class="mWt8p">중량</th>
                        <th class="mWt8p">판매단가</th>
                        <th class="mWt13p">관리</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                            ...
                            <div style="height: 500px; overflow-x: hidden; overflow-y: auto; margin: 0px; padding: 0px;">
                                <asp:DataGrid ID="grdTable" CssClass="grtable_td" runat="server" AllowCustomPaging="True" ShowHeader="False" AutoGenerateColumns="False" GridLines="both" PageSize="2" SelectedItemStyle-BackColor="#ccffff" Width="1190">
                                    ...
                                    <Columns>
                                        ...
                                        <asp:TemplateColumn HeaderText="">
                                            <ItemTemplate>
                                                <asp:Button ID="grd_btn_regist" CssClass="btn_60_20 btn_black" Text="등록" runat="server" />
                                                <asp:Button ID="grd_btn_updateList" CssClass="btn_60_20 btn_navy ml5" Text="변경이력" runat="server" />
                                            </ItemTemplate>
                                            <HeaderStyle HorizontalAlign="Center" />
